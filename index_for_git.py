...
def kospi_stock_market_tech(start_date: str, end_date: str):  # O(n^4)
    """
    For calculating the number of Listed, 52weeks High, 52weeks Low, Advancing(Declining) Equities Volumes
    A excel file will be exported and it is only for KOR data. US data is not needed because its data is from Bloomberg
    :param start_date: date start
    :param end_date: data end
    """
    date_list = pulling_dates_up(start_date, end_date)
    all_date_list = pulling_dates_up("1900-01-01", "2999-12-31")  # All days
    dataset_dict = {}
...
class Descriptors:
    def __init__(self, start_date: str, end_date: str):  # date form is 'yyyy-mm-dd'
        """
        Descriptors of Fear-and-Greed Index
        :param start_date: date start
        :param end_date: date end
        """
        self.date_list = pulling_dates_up(start_date, end_date)
        self.all_date_list = pulling_dates_up("1900-01-01", "2999-12-31")  # dates
...
class Scoring:
    def __init__(self, matrix: pd.DataFrame) -> pd.DataFrame:
        """
        how to calculate index from descriptors
        :param matrix: Descriptors of Index. Date and Price. N(Date) by 2
        """
        self.matrix = matrix
    ...
    def the_number_one(self):
        """
        Normalizing prices with mean, standard deviation in one year and the normalized is named as value.
        And, Value are valuated in the recent one year as percentile
        :return:
        """
        value = []
        ...
        for j in self.matrix.index:
            ...
            if j - relativedelta(weeks=52) >= self.matrix.index[len(self.matrix) - 1]:
                one_year_before = max(list(filter(lambda x: x <= one_year_before_date, self.matrix.index)))
                percentile.append((self.matrix.loc[j:one_year_before, 'Value'].rank(pct=True)[0])*100)  # Percentile
            else:
...
def index(start_date: str, end_date: str) -> pd.DataFrame:
    """
    index is the average of descriptor scores
    :param start_date: date start
    :param end_date: date end
    :return: pd.DataFrame
    """
    tem = Descriptors(start_date, end_date)
    ...
    for d in date_list:
        ...
        for i in percentile_matrix:
            if d in i.index:
                ...
                row.append(np.nan)
        row.append(np.nanmean(row[1:8]))  # mean
        columns.append(row)
